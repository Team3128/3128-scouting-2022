...
function gameReset(){
  if (confirm("Are you sure you want to reset?")) {
    ...
    //document.getElementById("transitionContainer").classList.add( "transitionAnimation" )
...
function reset() {
  ...
  fill_canvas(img);
  document.getElementById("transitionContainer").classList.remove( "transitionAnimation" )
  transitionState = false;
}
function startTimer() {
...
function autoClick(evt) {
  ...
  if (flipped) {
    autoPos[0] = 839 - autoPos[0];
    autoPos[1] = 348 - autoPos[1];
  }
...
function transitionPage(i) {
  ...
  if (i == "tele") { //auto to tele label change
    ...
    document.getElementById("counterLabel3").innerHTML = "0";
    //check if people are bad and forgot to do auto mobility
    if (dataValues[1] + dataValues[2] + dataValues[3] > 1) {
      dataValues[0] = true;
    }
  ...
  if (i == 2) { //qata to qr screen <--------------------------------- END SCREEN!
    if (dataValues[8] == 0 && dataValues[9] > 0) { //check if climb level selected
      if (!confirm("Are you sure you entered the climb level?")) {
        return;
      }
    }
    ...
    var points = 4*dataValues[1] + 2*dataValues[2] + 2*dataValues[4] + 1*dataValues[5];
    if (dataValues[0])
      points+=2;
    if (dataValues[8] == 4) {
      points+=15;
    } else if (dataValues[8] == 3) {
      points+=10;
    } else if (dataValues[8] == 2) {
      points+=6;
    } else if (dataValues[8] == 1) {
      points+=4;
    }
    console.log("Point total: " + points);
    document.getElementById("pointString").innerHTML = "Point total: " + points;
